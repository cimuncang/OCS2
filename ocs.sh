...
if [[ $vps = "FNS" ]]; then
	source="http://vps.imadenews.com"
else
	source="http://repo.imadenews.com"
fi
...
# check registered ip
wget -q -O daftarip http://www.imadenews.com/daftarip.txt
if ! grep -w -q $MYIP daftarip; then
	echo "Maaf, hanya IP yang terdaftar yang bisa menggunakan script ini!"
	if [[ $vps = "FNS" ]]; then
		echo "Hubungi: suryadewa bangsat(http://www.fornesia.com - 08563776008)"
	else
		echo "Hubungi: suryadewa (http://www.imadenews.com/contact/ - 08563776008)"
	fi
	rm -f /root/daftarip
	exit
fi
...
git init
git remote add origin https://github.com/suryadewa/ocs.git
...
echo "" | tee -a log-install.txt
echo "Auto Script Installer OCS Panels | suryadewa bangsat"  | tee -a log-install.txt
echo "suryadewa bangsat (http://www.imadenews.com/contact/ - 08563776008)"  | tee -a log-install.txt
echo "" | tee -a log-install.txt
echo "Thanks to Original Creator Ade Cikiciew (adenvt)" | tee -a log-install.txt
echo "" | tee -a log-install.txt
